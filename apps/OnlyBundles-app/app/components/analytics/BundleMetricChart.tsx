...
export function BundleMetricChart({
  trend,
  rangeDays,
  formatRevenue,
}: BundleMetricChartProps) {
  ...
  const selected =
    METRICS.find((option) => option.key === metric) ?? METRICS[0];
  ...
  useEffect(() => {
    ...
    const handleChange = (event: Event) => {
      ...
      if (!nextMetric || !METRICS.some((option) => option.key === nextMetric))
        return;
  ...
  return (
    <section
      className="wpb-card"
      aria-labelledby="wpb-bundle-metric-chart-title"
    >
      <header className="wpb-section-header wpb-section-header--metric-chart">
        <div>
          <h2 id="wpb-bundle-metric-chart-title" className="wpb-section-title">
            {translateAdmin(
              "adminExtracted.components.analytics.bundlemetricchart.bundleSplit"
            )}
          </h2>
          <p className="wpb-section-hint">
            {translateAdmin(
              "adminExtracted.components.analytics.bundlemetricchart.performanceAcrossTheSelectedDateRange"
            )}
          </p>
        </div>
        <div className="wpb-metric-selector">
          <s-button
            ...
            commandFor="bundle-metric-popover"
            variant="secondary"
          >
            {selected.label} {"▾"}
          </s-button>
          <s-popover id="bundle-metric-popover">
            <s-box padding="base">
              <s-choice-list
                ref={choiceListRef}
                name="bundle-metric"
                label={translateAdmin("adminAttributes.graphMetric")}
              ...
              >
                {METRICS.map((option) => (
                  <s-choice
                    key={option.key}
                    value={option.key}
                    selected={option.key === metric || undefined}
                  >
                    {option.label}
                  </s-choice>
                ))}
          ...
          >
            <defs>
              ...
              >
                <stop
                  offset="0%"
                  stopColor="var(--wpb-accent-engagement)"
                  stopOpacity={0.28}
                />
                <stop
                  offset="100%"
            ...
            <XAxis
              ...
              tickFormatter={(date: string) =>
                formatBundleSplitDateAxisTick(date, rangeDays)
              }
            />
            <YAxis
              axisLine={false}
            ...
            <Tooltip
              labelFormatter={(date) =>
                formatBundleSplitDateAxisTick(String(date), rangeDays)
              }
              labelStyle={{
                font: "var(--wpb-micro)",
