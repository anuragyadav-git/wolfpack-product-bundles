...
|---|---|---|---|---|
| 1 | Parse quantity selector visibility control | `parseConfiguration()` with `showQuantitySelectorOnCard` and dataset fallback | `config.showQuantitySelectorOnCard` is boolean with dataset fallback when control is missing | Covers `C13` shared-path evidence |
| 2 | Parse see-more and hover controls | `parseConfiguration()` with `displaySeeMoreLink` and `expandProductCardOnHover` in active controls | `config.displaySeeMoreLink` and `config.expandProductCardOnHover` are true | Covers `C14`/`C15` shared-path evidence |
...
## Acceptance Criteria
- [ ] Product-page config parser maps card visibility controls to canonical keys.
