...
| 22 | Constructor-started renderer lifecycle | The production widget constructor invokes the preview override before the derived constructor completes | The preview retains and awaits the exact initialization promise instead of losing it to class-field initialization | Prevents detached failures and false readiness |
| 23 | Successful local fixture initialization | FPB Standard and PPB List use deterministic preview fixtures | Each production controller finishes initialized without rendering its unavailable fallback | No Shopify, app-proxy, or storefront request |
| 24 | Transient renderer failure feedback | The trusted preview frame emits an ERROR event | One App Bridge error toast is shown and no persistent task banner is rendered | Uses the existing translated retry message |
...
- [x] Constructor-started initialization remains attached to the frame readiness lifecycle.
- [x] FPB and PPB deterministic fixtures initialize successfully through the production controllers.
- [x] Retryable preview failures use an App Bridge error toast rather than a persistent banner.
