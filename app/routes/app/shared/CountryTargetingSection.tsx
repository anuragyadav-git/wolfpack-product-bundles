...
import { buildShopifyCountryOptions } from "../../../lib/shopify-country-options";
    ...
    <s-section>
      <s-stack direction="block" gap="base">
        ...
        <s-banner tone="info" dismissible>
          <s-paragraph>
            {i18n.t("countryTargeting.shopifyOwnership")}
          </s-paragraph>
        </s-banner>
        <s-switch
        ...
        {state.countryCodes.length > 0 ? (
          <s-stack direction="inline" gap="small">
            {state.countryCodes.map((countryCode) => {
              const countryLabel =
                labelByCountryCode.get(countryCode) ?? countryCode;
              return (
                <s-clickable-chip
                  key={countryCode}
                  removable
