...
interface OfferOperationsSectionProps {
  ...
  onScheduleModeChange: (mode: OfferOperationsAdminState['scheduleMode']) => void;
...
export function OfferOperationsSection({
  ...
  const statusTone = schedule.state === 'active'
    ? 'success'
    ...
    <s-section>
      <s-stack direction="block" gap="base">
        ...
        <s-stack direction="inline" gap="small" alignItems="center">
          ...
          <s-badge tone={statusTone}>{i18n.t(`offerOperations.status.${schedule.state}`)}</s-badge>
        </s-stack>
        {schedule.nextTransitionAt ? (
          <s-text>{i18n.t('offerOperations.nextTransition', {
        ...
        <s-select
          ...
          onChange={(event) => onScheduleModeChange(
            event.currentTarget.value as OfferOperationsAdminState['scheduleMode'],
          )}
        >
          <s-option value="always">{i18n.t('offerOperations.scheduleModeAlways')}</s-option>
          <s-option value="one_time">{i18n.t('offerOperations.scheduleModeOneTime')}</s-option>
          <s-option value="recurring">{i18n.t('offerOperations.scheduleModeRecurring')}</s-option>
        </s-select>
        ...
        {state.scheduleMode === 'recurring' ? (
          <>
            ...
            <s-select
              ...
              onChange={(event) => onRecurrenceFrequencyChange(
                event.currentTarget.value as OfferOperationsAdminState['recurrenceFrequency'],
              )}
            >
              <s-option value="weekly">{i18n.t('offerOperations.recurrenceWeekly')}</s-option>
              <s-option value="monthly">{i18n.t('offerOperations.recurrenceMonthly')}</s-option>
            </s-select>
            <s-date-field
              ...
              onInput={(event) => onRecurrenceAnchorDateChange(
            ...
            <s-grid gridTemplateColumns="repeat(auto-fit, minmax(12rem, 1fr))" gap="base">
              <s-text-field
                ...
                placeholder={i18n.t('offerOperations.recurrenceTimePlaceholder')}
                value={state.recurrenceWindowStart ?? ''}
                onInput={(event) => onRecurrenceWindowStartChange(
              ...
              <s-text-field
                ...
                onInput={(event) => onRecurrenceWindowEndChange(
                  (event.target as HTMLInputElement).value || null,
                )}
              />
            </s-grid>
            <s-select
              ...
              onChange={(event) => onRecurrenceTerminationChange(
                event.currentTarget.value as OfferOperationsAdminState['recurrenceTermination'],
              )}
            >
              <s-option value="never">{i18n.t('offerOperations.recurrenceNever')}</s-option>
              <s-option value="on_date">{i18n.t('offerOperations.recurrenceOnDate')}</s-option>
              <s-option value="after_runs">{i18n.t('offerOperations.recurrenceAfterRuns')}</s-option>
            </s-select>
            {state.recurrenceTermination === 'on_date' ? (
              <s-date-field
                label={i18n.t('offerOperations.recurrenceEndsOnLabel')}
                value={state.recurrenceEndsOn ?? ''}
                onInput={(event) => onRecurrenceEndsOnChange(
                  event.currentTarget.value || null,
                )}
              />
            ) : null}
            {state.recurrenceTermination === 'after_runs' ? (
              <s-number-field
                ...
                min={1}
                value={state.recurrenceRunCount == null ? '' : String(state.recurrenceRunCount)}
                onInput={(event) => {
