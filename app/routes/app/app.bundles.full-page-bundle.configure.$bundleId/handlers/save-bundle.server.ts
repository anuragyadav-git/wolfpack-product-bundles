...
async function validatePersistedStepProductVariants(
  ...
  for (const reference of references) {
    ...
    return json(
      {
        success: false,
        error: `fpb-save blocked variant in step ${reference.stepIndex + 1}, product ${reference.productIndex + 1}: ${toStringVariant(reference.rawVariantId)} is not available on storefront (${status}).`,
        context: {
          route: "fpb-save",
