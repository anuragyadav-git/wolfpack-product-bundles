...
import { LANGUAGE_CONFIGURATION } from "../../../lib/admin-configuration-surfaces";
      ...
      <s-page inlineSize="large">
        <s-stack gap="@container (inline-size > 600px) large, base">
          <s-stack direction="inline" gap="small" alignItems="center">
            ...
            <s-heading>
              {translateAdmin(
                "adminExtracted.appSettings.languagesettingsview.languageSettings"
              )}
            </s-heading>
          </s-stack>
              ...
              >
                <s-switch
                  label={translateAdmin(
                    "adminAttributes.enableMultipleLanguages"
                  )}
                  details="Show bundle labels in more than one language."
                  checked={languageMode === "MULTIPLE" || undefined}
                  onChange={(event) =>
                    onModeChange(
                      event.currentTarget.checked ? "MULTIPLE" : "SINGLE"
                    )
                  }
                />
                <s-select
                  label={translateAdmin("adminAttributes.addOrEditLanguages")}
                  ...
                  onChange={(event) => {
                    ...
                    onSelectLocale(
                      locale,
                      localeFieldValues[locale]
                        ? undefined
                        : getInitialLanguageFieldValues(locale)
                    );
                  }}
                >
                  {SETTINGS_LANGUAGE_LOCALES.map((language) => (
                    <s-option key={language.code} value={language.code}>
                      {translateAdminCopy(language.label)}
                    </s-option>
                  ))}
                </s-select>
              </s-grid>
              <div className={styles.languageLocaleRail}>
                <s-stack direction="inline" gap="small" alignItems="center">
                  {Object.keys(localeFieldValues).map((locale) => {
                    const label =
                      SETTINGS_LANGUAGE_LOCALES.find(
          ...
          <div className={styles.languageContentLayout}>
            <s-section>
              <s-stack gap="large">
                <s-stack gap="small">
                  <s-heading>
                    {translateAdmin(
                      "adminExtracted.appSettings.languagesettingsview.sharedLabels"
                    )}
                  </s-heading>
                  <s-text color="subdued">
                    {translateAdmin(
                      "adminExtracted.appSettings.languagesettingsview.usedAcrossEveryBundleLayout"
                    )}
                  </s-text>
                  <div className={styles.languageDesktopPanelNav}>
                    <s-button
                      variant={
                        activePanel === "cartCheckout" ? "primary" : "tertiary"
                      }
                      onClick={() => onPanelChange("cartCheckout")}
                    >
                      {translateAdmin(
                        "adminExtracted.appSettings.languagesettingsview.cartAmpCheckout"
                      )}
                    </s-button>
                  </div>
                </s-stack>
                ...
                <s-stack gap="small">
                  <s-heading>
                    {translateAdmin(
                      "adminExtracted.appSettings.languagesettingsview.bundleLabels"
                    )}
                  </s-heading>
                  <s-text color="subdued">
                    {translateAdmin(
                      "adminExtracted.appSettings.languagesettingsview.chooseALayoutThenEditItsLabels"
                    )}
                  </s-text>
                  <s-select
                    label={translateAdmin("adminAttributes.bundleLayout")}
                    value={activeLayout}
                    onChange={(event) =>
                      onLayoutChange(event.currentTarget.value || activeLayout)
                    }
                  >
                    <s-option value="Landing Page Layout">
                      {translateAdmin(
                        "adminExtracted.appSettings.languagesettingsview.landingPageLayout"
                      )}
                    </s-option>
                    <s-option value="Product Page Layout">
                      {translateAdmin(
                        "adminExtracted.appSettings.languagesettingsview.productPageLayout"
                      )}
                    </s-option>
                  </s-select>
                  <div className={styles.languageDesktopPanelNav}>
                    {sections.map((section) => (
                      <s-button
                        key={section}
                        variant={
                          activePanel === section ? "primary" : "tertiary"
                        }
                  ...
                  <nav
                    className={styles.languageMobilePanelRail}
                    aria-label={translateAdmin(
                      "adminAttributes.languageSections"
                    )}
                  >
                    <s-stack direction="inline" gap="small">
                      <s-clickable-chip
                        color={
                          activePanel === "cartCheckout" ? "strong" : "base"
                        }
                        onClick={() => onPanelChange("cartCheckout")}
                      >
                        {translateAdmin(
            ...
            <s-stack gap="base">
              {fieldGroups.map((group) => {
                const variables = getSettingsVariables(
                  group.fields,
                  fieldValues
                );
                return (
                  <s-section
                    key={`${activePanel}-${group.title}`}
                    heading={translateAdminCopy(group.title)}
                  >
                    <s-stack gap="base">
                      <s-stack
                        direction="inline"
                      ...
                      >
                        <s-text color="subdued">
                          {group.description
                            ? translateAdminCopy(group.description)
                            : undefined}
                        </s-text>
                        {variables.length > 0 && (
                          <s-button
                            ...
                            command="--show"
                            onClick={() =>
                              onShowVariables(group.title, variables)
                            }
                          >
                            {translateAdmin(
