...
renderProductPageLayout() {
  ...
  if (usesCascadeStepFlow && this._isProductPageGridTemplate?.() === true) {
    this._renderGridStepFlowLayout();
    return;
  }
...
_renderGridStepFlowLayout() {
  const sequence = getGridStepRenderSequence({
...
_createInpageStepSection(step, stepIndex) {
  ...
  const sectionClass = [
    'bw-ppb-inpage-step-section',
    preset ? `bw-ppb-inpage-step-section--${preset.toLowerCase()}` : null,
    isCascade ? 'wpbMixCascadeBodyWrapper' : null,
  ].filter(Boolean).join(' ');
