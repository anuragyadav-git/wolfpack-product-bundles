// Shopify CountryCode values from the installed Shopify UI/Function schemas.
// `ZZ` is intentionally omitted because it represents an unknown region, not a
// merchant-selectable targeting country.
export const SHOPIFY_COUNTRY_CODES = [
  'AC', 'AD', 'AE', 'AF', 'AG', 'AI', 'AL', 'AM', 'AN', 'AO', 'AR', 'AT',
  ...
  'GN', 'GP', 'GQ', 'GR', 'GS', 'GT', 'GW', 'GY', 'HK', 'HM', 'HN', 'HR',
...
export function buildShopifyCountryOptions(locale: string): ShopifyCountryOption[] {
  const displayNames = new Intl.DisplayNames([locale], { type: 'region' });
  return SHOPIFY_COUNTRY_CODES
    .map((code) => {
      const label = displayNames.of(code) ?? code;
      return {
        code,
        label,
        searchValue: `${label} ${code}`.toLocaleLowerCase(locale),
      };
    })
