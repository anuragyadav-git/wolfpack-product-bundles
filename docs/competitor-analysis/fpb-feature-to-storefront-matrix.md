...
| `F0-MOBILE-TOGGLE` | Fresh EB mobile proof at `390x844` established the summary toggle contract: one centered black quantity pill shows an upward filled caret while collapsed and a downward caret while expanded. WPB previously reversed those directions, duplicated the icon across preset styles, omitted Horizontal ownership, rendered a second Classic-only toggle row, and used a narrower pseudo-element control. Widget `5.0.220` renders one stable semantic button with the same real `20px` SVG, `10px` inline padding, approximately `47.8 x 25.2` geometry, and synchronized `300ms` caret and tray motion. Cache-bypassed Horizontal replay confirmed the same button node remained connected, `aria-expanded` changed in place, the footer interpolated from `85.5px` to `337.3px` instead of jumping, collapse reversed the transition, and document overflow remained zero. | Direct Chrome DevTools MCP accessibility, computed-style, resource, geometry, timing, network, and screenshot evidence from the 2026-07-30 sweep; `app/assets/widgets/full-page/methods/mobile-summary-methods.js`; `app/assets/widgets/full-page-css/shared/mobile-summary-footer.css`; `tests/unit/assets/fpb-standard-mobile-summary-action.test.ts`. |
