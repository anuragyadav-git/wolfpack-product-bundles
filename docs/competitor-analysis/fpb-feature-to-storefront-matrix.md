...
| `F0-MOBILE-TOGGLE` | Fresh EB mobile proof at `390x844` established the summary toggle contract: one centered quantity pill shows an upward filled caret while collapsed and a downward caret while expanded. WPB previously reversed those directions, duplicated the icon across preset styles, omitted Horizontal ownership, rendered a second Classic-only toggle row, and used a narrower pseudo-element control. A final live replay found residual CSS drift: the shared pill could shrink below the EB-sized primitive, Standard, Classic, and Compact overrode the shared minimum with `max-content`, and the tray still used `360ms` while its caret and animation lifecycle used `300ms`. The source now holds every preset pill at a `48px` minimum and synchronizes the tray, caret, and lifecycle at `300ms`. Cache-bypassed Horizontal replay on widget `5.0.222` confirmed the same button node remained connected, `aria-expanded` changed in place, the width remained `48px`, the footer expanded from `85.5px` to `337.2px`, and collapse returned it to `85.5px`. Served dev-tunnel CSS inspection confirmed Standard, Classic, and Compact no longer emit the conflicting `max-content` minimum. | Direct Chrome DevTools MCP accessibility, computed-style, resource, geometry, timing, network, and screenshot evidence from the 2026-07-30 sweep; `app/assets/widgets/full-page/methods/mobile-summary-methods.js`; `app/assets/widgets/full-page-css/base/sidebar-totals-discounts.css`; `app/assets/widgets/full-page-css/shared/mobile-summary-footer.css`; `app/assets/widgets/full-page-css/templates/side-footer-standard.css`; `app/assets/widgets/full-page-css/templates/side-footer-compact.css`; `app/assets/widgets/full-page-css/templates/classic/mobile.css`; `tests/unit/assets/fpb-standard-mobile-summary-action.test.ts`. |
