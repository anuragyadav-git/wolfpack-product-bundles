...
| `F0-MOBILE-TOGGLE` | Fresh EB mobile proof at `390x844` established the summary toggle contract: one centered black quantity pill shows an upward filled caret while collapsed and a downward caret while expanded. WPB previously reversed those directions, duplicated the icon across preset styles, omitted Horizontal ownership, rendered a second Classic-only toggle row, and used a narrower pseudo-element control. A final live replay found a residual shared-CSS mismatch: the pill could shrink below the EB-sized primitive and the tray still used `360ms` while its caret and animation lifecycle used `300ms`. The source now holds the pill at a `48px` minimum and synchronizes the tray, caret, and lifecycle at `300ms`. Cache-bypassed Horizontal replay on widget `5.0.222` confirmed the same button node remained connected, `aria-expanded` changed in place, the footer interpolated from `85.5px` to `337.3px`, collapse reversed the transition, and document overflow remained zero. | Direct Chrome DevTools MCP accessibility, computed-style, resource, geometry, timing, network, and screenshot evidence from the 2026-07-30 sweep; `app/assets/widgets/full-page/methods/mobile-summary-methods.js`; `app/assets/widgets/full-page-css/base/sidebar-totals-discounts.css`; `app/assets/widgets/full-page-css/shared/mobile-summary-footer.css`; `tests/unit/assets/fpb-standard-mobile-summary-action.test.ts`. |
