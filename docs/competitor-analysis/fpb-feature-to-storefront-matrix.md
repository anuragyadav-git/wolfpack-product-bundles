...
| `F3-FIXED-PRICE-EB` | EB was configured with one exact-quantity fixed bundle price rule, `2 products -> 1000`, with progress disabled and fixed-price messaging enabled. Standard proved the complete quantity contract: zero and one item showed the remaining-product message, exactly two reduced `1158.00` to `1000.00` with a `14% off` badge and success copy, and three items did not qualify. Standard showed the success message in its desktop footer; Classic, Compact, and Horizontal omitted that message from the desktop sidebar while retaining the fixed total and badge. All four presets reused the same mobile footer contract with success copy, fixed total, badge, and quantity. EB was restored to Horizontal with percentage tiers `2 -> 10%` and `3 -> 20%`, progress disabled, and messaging enabled. WPB cells remain non-terminal because the equivalent fixture save POST failed before reaching the app with `net::ERR_NAME_NOT_RESOLVED` for the user-provided dev tunnel; the persisted WPB fixture and storefront remained at F0. | Direct Chrome DevTools MCP snapshots and runtime probes from the 2026-07-26 EB sweep; WPB preserved-network request evidence from the failed configure save. |
