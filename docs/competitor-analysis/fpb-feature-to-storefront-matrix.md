...
| `F7-CART-CONTROLS-PARTIAL` | WPB now exposes and persists the Landing Page `Execute Script` control, and the qualified Horizontal add-on flow ran its saved `sessionStorage` counter exactly once before redirecting to `/cart`. The same live add produced one transformed `Daily Essentials` parent line with four component identities, `$2606.00` retail, `$10.00` savings, `$2596.00` final, and synchronized cart details. The saved amount-only control reached the app runtime, but the active dev-preview Cart Transform still emitted `You Save: $10.00 (0%)` after a cache-bypassed replay and a Function watcher refresh. G21 and the remaining F7 cells therefore stay non-terminal until the dev Function input consumes the saved `bundle_cart_line_messaging` metafield and the all-preset redirect/inventory sweep is completed. | Direct Chrome DevTools MCP Settings save request, Controls API response, qualified Horizontal storefront/cart interaction, `/cart/add.js`, `/cart.js`, session storage, and Function watcher replay from 2026-07-30; `tests/unit/assets/fpb-post-add-controls.test.ts`; `tests/unit/lib/admin-configuration-surfaces.test.ts`; `tests/unit/lib/settings-controls-runtime.test.ts`; `test-spec/fpb-post-add-controls.spec.md`. |
