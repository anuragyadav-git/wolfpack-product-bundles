...
| `F3-FIXED-AMOUNT-SIMPLE-ALL` | EB and WPB were configured with a fixed amount discount at an amount threshold, simple progress, discount messaging, and custom success copy, then cycled through all four presets. Qualified WPB mobile summaries reused the shared footer state and progress primitive while preset CSS supplied only measured presentation deltas; Horizontal also retained the shared desktop summary, Add Ons, and product-slot flow. The sweep exposed and repaired a bundled-widget declaration collision and a shared mobile-progress visibility defect. Both fixtures were restored to Horizontal with percentage tiers `2 -> 10%` and `3 -> 20%`, progress disabled, messaging enabled, and original copy. | `/private/tmp/fpb-feature-parity/F3/eb/fixed-amount/simple-progress/all-presets-delta.md`, `test-spec/fpb-bundled-widget-syntax.spec.md` |
| `F3-FIXED-PRICE-PARTIAL` | EB was configured with one exact-quantity fixed bundle price rule, `2 products -> 1000`, with progress disabled and fixed-price messaging enabled. Standard proved the complete quantity contract: zero and one item showed the remaining-product message, exactly two reduced `1158.00` to `1000.00` with a `14% off` badge and success copy, and three items did not qualify. Standard showed the success message in its desktop footer; Classic, Compact, and Horizontal omitted that message from the desktop sidebar while retaining the fixed total and badge. All four presets reused the same mobile footer contract with success copy, fixed total, badge, and quantity. WPB then saved and rendered the equivalent exact-quantity fixture. Horizontal desktop and mobile and Standard desktop reduced `$1448.00` to `$1000.00`; Standard desktop showed success copy while Horizontal desktop omitted it and Horizontal mobile showed it, matching the observed EB surface ownership. The missing WPB badge was fixed in `0df9da4c` by rendering the canonical `discountInfo.discountPercentage` through the shared desktop sidebar and mobile footer. Classic, Compact, and the post-build badge states remain unswept, so no additional matrix cells were promoted. Both fixtures were restored to Horizontal with percentage tiers `2 -> 10%` and `3 -> 20%`, progress disabled, and messaging enabled; WPB's exact step rule was restored to `1`. | Direct Chrome DevTools MCP snapshots and runtime probes from the 2026-07-26 EB and WPB sweeps; `tests/unit/assets/fpb-summary-discount-badge.test.ts`; `test-spec/fpb-summary-discount-badge.spec.md`. |
