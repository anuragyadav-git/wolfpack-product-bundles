...
describe('PPB category-scoped variants', () => {
  it('keeps only explicitly configured variants and promotes the survivor to card identity', () => {
    const step = {
      categories: [{
        categoryId: 'cat-10-only',
        products: [{
          selectionId: 'gid://shopify/Product/101',
          variants: [{ selectionId: 'gid://shopify/ProductVariant/1002' }],
        }],
      }],
    };
  ...
  it('keeps every hydrated variant when the category has no explicit subset', () => {
    const step = {
      categories: [{
        categoryId: 'cat-all-variants',
        products: [{ selectionId: 'gid://shopify/Product/101', variants: [] }],
      }],
    };
