...
function main() {
  ...
  const coverage = parseCsv(new URL('../00-inventory/configuration-coverage.csv', import.meta.url));
  ...
  console.log(`Extracted ${payload.count} configuration rows from configuration-registry.`);
  if (uncovered.length > 0) {
    console.log('Missing configuration coverage entries:');
