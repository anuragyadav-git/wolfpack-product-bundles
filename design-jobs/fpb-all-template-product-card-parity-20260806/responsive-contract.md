---
schema_version: 1
id: fpb-all-template-product-card-responsive-contract
...
| VP-02 | 360 | 800 | Full widget container | Baseline mobile comparison | SUM-01 through SUM-20 where mobile applies |
...
| VP-05 | 768 | 1024 | Full widget container | Tablet portrait and CSS boundary check | Shared tray, expanded review, keyboard, 200% zoom |
...
| SUM-REVIEW | Container < 1024 collapsed | Zero visual/interactive height | Collapsed grid track | Hidden between persistent status and action | `inert` and `aria-hidden`; no focusable descendants | None | None | Not painted | No hidden control consumes space | Not applicable | Collapsed cleanly without page jump |
| SUM-REVIEW | Container < 1024 expanded | Bounded by remaining dynamic viewport space | One-column review | Opens between status and persistent action | Visible and operable | Tray review becomes the internal scroll owner only when needed | Tray remains sticky; review itself not fixed | Long product/variant copy wraps; media remains bounded | Rows and slots use shared mobile spacing; no per-preset geometry branch | Top/bottom inset preserved | Overscroll contained in review; page remains scrollable outside interaction |
...
| SUM-TOTAL | < 1024 | Intrinsic within CTA plus expanded detail when required | Action label, separator, final price | Persistent bottom action | Final price always visible; original/savings reachable expanded | None | Part of sticky tray action | Label and price remain readable at 320px | CTA fills available inner width | Bottom safe-area padding | No collision with disclosure |
...
|---|---|---|
| B-01 | Container 1023 / 1024 / 1025 | Direction-A replacement boundary: 1023 uses one shared tray; 1024 and 1025 use the preset desktop sidebar. No frame may show both or neither. |
| B-02 | Viewport 767 / 768 / 769 | Tablet-boundary test: shared tray may remain active in this range if container < 1024; summary semantics must remain one tray. |
| B-03 | Viewport 319 / 320 / 321 | 320 is the minimum supported proof width; 319 is a diagnostic overflow probe and must fail gracefully without hiding primary action. |
...
Approved RD-001: use the measured FPB widget-container width, with desktop sidebar at `>= 1024px` and shared tray below `1024px`. Rationale: this preserves the current Standard/Classic usable boundary, resolves the current 768/1024 conflict, supports constrained themes and 200% zoom, and avoids forcing a two-column sidebar into tablet portrait. Aditya Awasthi approved this behavior on 2026-08-04.
...
- Focus during replacement: if focused control disappears at the boundary, move focus to semantic counterpart only when resize is user-driven and focus would otherwise be lost.
- 200% zoom: effective container may cross RD-001 and use the tray. This is valid responsive reflow, not desktop regression.
...
- State preservation: crossing either direction never clears selections, changes price, changes active step, repeats add-on pulses, or duplicates event listeners.
