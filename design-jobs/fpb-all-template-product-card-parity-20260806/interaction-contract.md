---
schema_version: 1
id: fpb-all-template-product-card-interaction-contract
...
|---|---|---|---|---|---|---|---|---|---|
| INT-01 | Mobile summary disclosure `button` | Stable localized equivalent of “Review your bundle”; quantity/status is supplementary, not the sole name | Tap toggles the same connected tray | Native Enter/Space | Toggles `aria-expanded`; review region toggles `inert` and `aria-hidden` | Focus stays on disclosure after toggle; collapsing while focus is inside first returns focus to disclosure | Never disabled; remains operable during empty, loading, error, and qualification states | If content cannot hydrate, disclosure remains stable and error status is exposed without trapping focus | Same DOM control persists; caret/tray animate only when motion is allowed |
